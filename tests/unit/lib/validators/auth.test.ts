 ...
 * 테스트 대상: lib/validators/auth.ts (아직 구현 없음 → FAIL 예상)
...
describe('validateEmail', () => {
  ...
  it('@가 없는 이메일에 형식 오류를 반환한다', () => {
    expect(validateEmail('userexample.com')).toBe('올바른 이메일 형식을 입력해주세요')
  })

  // U-30: 도메인 없는 이메일
  it('도메인이 없는 이메일(user@)에 형식 오류를 반환한다', () => {
    expect(validateEmail('user@')).toBe('올바른 이메일 형식을 입력해주세요')
  })

  it('형식이 잘못된 이메일(not-an-email)에 오류를 반환한다', () => {
    expect(validateEmail('not-an-email')).toBe('올바른 이메일 형식을 입력해주세요')
  })
})
...
describe('validatePassword', () => {
  ...
  it('7자 비밀번호(Abcdef1)에 8자 미만 오류를 반환한다', () => {
    expect(validatePassword('Abcdef1')).toBe('비밀번호는 8자 이상이어야 합니다')
  })

  // B-04: 경계값 - 7자
  it('7자 비밀번호에 오류를 반환한다', () => {
    expect(validatePassword('short12')).toBe('비밀번호는 8자 이상이어야 합니다')
  })
  ...
  it('숫자가 없는 8자 이상 비밀번호에 영문+숫자 조합 오류를 반환한다', () => {
    expect(validatePassword('abcdefgh')).toBe('비밀번호는 영문과 숫자를 모두 포함해야 합니다')
  })

  it('영문이 없는 비밀번호에 영문+숫자 조합 오류를 반환한다', () => {
    expect(validatePassword('12345678')).toBe('비밀번호는 영문과 숫자를 모두 포함해야 합니다')
  })
})
...
describe('validateConfirmPassword', () => {
  ...
  it('abcd1234와 abcd5678이 일치하지 않으면 오류를 반환한다', () => {
    expect(validateConfirmPassword('abcd1234', 'abcd5678')).toBe('비밀번호가 일치하지 않습니다')
  })
})
...
describe('getAuthErrorMessage', () => {
  // U-34: Supabase 에러 → 한국어 변환
  it('"Invalid login credentials" 에러를 한국어로 변환한다', () => {
    expect(getAuthErrorMessage('Invalid login credentials')).toBe(
      '이메일 또는 비밀번호가 올바르지 않습니다'
    )
  })

  it('"Email not confirmed" 에러를 한국어로 변환한다', () => {
    expect(getAuthErrorMessage('Email not confirmed')).toBe(
      '이메일 인증을 완료해주세요. 메일함을 확인하세요'
    )
  })

  it('"User already registered" 에러를 한국어로 변환한다', () => {
    expect(getAuthErrorMessage('User already registered')).toBe(
      '이미 가입된 이메일입니다. 로그인해주세요'
    )
  })

  it('rate limit 에러를 한국어로 변환한다', () => {
    expect(
      getAuthErrorMessage(
        'For security purposes, you can only request this once every 60 seconds'
      )
    ).toBe('잠시 후 다시 시도해주세요 (1분 제한)')
  })

  // U-35: 알 수 없는 에러
  it('알 수 없는 에러에 기본 메시지를 반환한다', () => {
    expect(getAuthErrorMessage('unknown error xyz')).toBe(
      '일시적인 오류가 발생했습니다. 다시 시도해주세요'
    )
  })

  it('빈 문자열 에러에 기본 메시지를 반환한다', () => {
    expect(getAuthErrorMessage('')).toBe('일시적인 오류가 발생했습니다. 다시 시도해주세요')
  })
})
